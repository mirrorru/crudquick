//go:build smoke || crudsgen || functional
//go:generate crudsgen -src=.:* -dest=test/testmodels -pkg=testmodels
// Code generated by crudsgen v(devel); DO NOT EDIT.

package testmodels

import (
	"context"
	"errors"
	"strings"

	"github.com/mirrorru/cruds"
	"github.com/mirrorru/cruds/defs"
	"github.com/mirrorru/cruds/dialect"
	"github.com/mirrorru/cruds/struct_info"
)

var _ cruds.TypedTable[ProductRow] = (*TableProductRow)(nil)

type TableProductRow struct {
	dialect   dialect.SQLDialect
	tableInfo struct_info.TableInfo
	sqlTexts  struct_info.SqlTexts
}

func (t *TableProductRow) Fix(ctx context.Context, tx cruds.TxProcessor, row *ROW, fixFieldIdx []int)  (Result, error) {
	//TODO implement me
	panic("implement me")
}

type tableProductRowInternals struct {
	TableInfo struct_info.TableInfo
	SqlTexts  struct_info.SqlTexts
}

func NewTableProductRowVal(d dialect.SQLDialect) TableProductRow {
	tableInfo := struct_info.TableInfo{
		SQLName: "products",
		Fields: struct_info.TableFields{
			{Path: []string{"ID" }, SQLName: "id", IsPK: true, CanSelect: true},
			{Path: []string{"Name" }, SQLName: "name", CanSelect: true, CanInsert: true, CanUpdate: true, SortPos: 1},
			{Path: []string{"Price" }, SQLName: "price", CanSelect: true, CanInsert: true, CanUpdate: true},
			{Path: []string{"Stock" }, SQLName: "stock", CanSelect: true, CanInsert: true, CanUpdate: true},
		},
		FieldNameIdx:  map[string]int{"id": 0, "name": 1, "price": 2, "stock": 3 },
		PKIdxList:     []int{0 },
		InsertIdxList: []int{1, 2, 3 },
		UpdateIdxList: []int{1, 2, 3 },
		SelectIdxList: []int{0, 1, 2, 3 },
		SortIdxList:   []int{1 },
		RefIdxList:    []int{ },
	}

	return TableProductRow{
		dialect:   d,
		tableInfo: tableInfo,
		sqlTexts:  struct_info.SqlBuilderVal.SQLTexts(d, &tableInfo),
	}
}

func NewTableProductRow(d dialect.SQLDialect) *TableProductRow {
	return new(NewTableProductRowVal(d))
}

func (t *TableProductRow) Internals() tableProductRowInternals {
	return tableProductRowInternals{
		TableInfo: t.tableInfo,
		SqlTexts:  t.sqlTexts,
	}
}

func (t *TableProductRow) scanRefs(buf *ProductRow) []any {
	return []any{&buf.ID, &buf.Name, &buf.Price, &buf.Stock}
}

func (t *TableProductRow) insertArgs(row *ProductRow) []any {
	return []any{row.Name, row.Price, row.Stock}
}

func (t *TableProductRow) updateArgs(row *ProductRow) []any {
	return []any{row.Name, row.Price, row.Stock, row.ID}
}

func (t *TableProductRow) Ins(ctx context.Context, tx cruds.TxProcessor, row *ProductRow) (*ProductRow, cruds.Result, error) {
	args := t.insertArgs(row)
	if !t.dialect.SupportsReturning() {
		sqlResult, err := tx.ExecContext(ctx, t.sqlTexts.Insert, args)
		return row, sqlResult, err
	}
	buf := new(ProductRow)
	refs := t.scanRefs(buf)
	err := tx.QueryRowContext(ctx, t.sqlTexts.Insert, args...).Scan(refs...)

	return buf, nil, err
}

func (t *TableProductRow) Upd(ctx context.Context, tx cruds.TxProcessor, row *ProductRow) (*ProductRow, cruds.Result, error) {
	args := t.updateArgs(row)
	if !t.dialect.SupportsReturning() {
		sqlResult, err := tx.ExecContext(ctx, t.sqlTexts.Update, args)
		return row, sqlResult, err
	}
	buf := new(ProductRow)
	refs := t.scanRefs(buf)
	err := tx.QueryRowContext(ctx, t.sqlTexts.Update, args...).Scan(refs...)

	return buf, nil, err
}

func (t *TableProductRow) One(ctx context.Context, tx cruds.TxProcessor, keys ...any) (*ProductRow, error) {
	buf := new(ProductRow)
	refs := t.scanRefs(buf)
	err := tx.QueryRowContext(ctx, t.sqlTexts.GetOne, keys...).Scan(refs...)

	return buf, err
}

func (t *TableProductRow) Del(ctx context.Context, tx cruds.TxProcessor, keys ...any) (cruds.Result, error) {
	return tx.ExecContext(ctx, t.sqlTexts.Delete, keys...)
}

func (t *TableProductRow) Many(ctx context.Context, tx cruds.TxProcessor, filter *cruds.Filter) (result []*ProductRow, err error) {
	var (
		query strings.Builder
		args  []any
		where string
	)
	query.WriteString(t.sqlTexts.ListStart)

	if filter != nil {
		if filter.Range != nil {
			var argIdx int
			where, args, err = filter.Range.Build(t.tableInfo.Fields, t.dialect, &argIdx)
			if err != nil {
				return nil, err
			}
			query.WriteString(defs.SQLWhere)
			query.WriteString(where)
		}
	}
	query.WriteString(t.sqlTexts.SortPart)
	if filter != nil {
		query.WriteString(t.dialect.OffsetAndLimit(filter.Offset, filter.Limit))
	}
	buf := new(ProductRow)
	refs := t.scanRefs(buf)

	rows, err := tx.QueryContext(ctx, query.String(), args...)
	if err != nil {
		return nil, err
	}
	defer func() {
		err = errors.Join(err, rows.Close())
	}()

	for rows.Next() {
		if err = rows.Scan(refs...); err != nil {
			return nil, err
		}
		rec := new(ProductRow)
		*rec = *buf
		result = append(result, rec)
	}

	if err = rows.Err(); err != nil {
		return nil, err
	}

	return result, nil
}
