...
type FilterNode interface {
	Build(tf struct_info.TableFields, d dialect.SQLDialect, argIdx *int) (clause string, args []any, err error)
}
...
func (f *Filter) BuildOffsetAndLimit(d dialect.SQLDialect) string {
...
func (f *Filter) BuildWhere(tf struct_info.TableFields, d dialect.SQLDialect) (query string, args []any, err error) {
...
func (cn ConditionNode) Build(tf struct_info.TableFields, d dialect.SQLDialect, argIdx *int) (string, []any, error) {
...
func (gn GroupNode) Build(tf struct_info.TableFields, d dialect.SQLDialect, argIdx *int) (string, []any, error) {
	switch gn.Logic {
...
// Cond создаёт узел условия: fieldIdx — индекс поля вstruct_info.TableFields, op — оператор, value — значение.
// EN: Cond creates a condition node: fieldIdx — field index instruct_info.TableFields, op — operator, value — value.
